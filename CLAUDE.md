...
```
SIT (20m) ──▶ STAND (8m) ──▶ WALK (2m) ──▶ SIT ...   (ค่าตั้งต้น = กฎ 20-8-2)
```

**Alarms 3 ตัว:**
- `phase-timer` — one-shot ตั้งตอนเข้าเฟสใหม่ ยิงเมื่อหมดเวลา → `advancePhase()`
- `tick` — ทุก 1 นาที อัปเดต badge นับถอยหลังบนไอคอน
- `resume` — ตั้งตอน snooze เพื่อปลุกกลับเฟสเดิม

**Storage 2 keys:**
- `state` = `{ running, paused, phase, phaseEndsAt, cycle }`
- `settings` = `{ durations{SIT,STAND,WALK}, soundEnabled, snoozeMinutes, workHoursEnabled, startHour, endHour }`
...
| `background.js` | state machine + alarms + notifications | แก้ logic การเตือนทั้งหมด |
